...
command = File.separator+cmtkpath+File.separator+"munger -b "+cmtkpath+" "+a+w+r+" "+c1+c2+c3+"  -X "+X+" -C "+C+" -G "+G+" -R "+R+" -A '--accuracy "+A+"' -W '--accuracy "+A+"'  -v -T 80 -s "+refbrain+" images";
...
exec("sh", "-c", "cd "+workdir+File.separator+name+"Registration; start=`date +%s`; "+cmtkpath+File.separator+"munger -b "+cmtkpath+" "+a+w+r+" "+c1+c2+c3+"  -X "+X+" -C "+C+" -G "+G+" -R "+R+" -A '--accuracy "+A+"' -W '--accuracy "+A+"'  -v -T 80 -s "+refbrain+" images"+"; end=`date +%s`; echo Execution time was `expr $end - $start` seconds.");
